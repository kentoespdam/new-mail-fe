...
---
**Instruction:** Follow Epic roadmap (1-6) and use `context7` for latest TanStack/Next.js best practices. Standby for task execution. always use bun for scripts (e.g., `bun run dev`). always use `bun run lint` and `bun run format` before committing code.